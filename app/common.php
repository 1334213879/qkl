...
function getweek($id){
    return db('restoration_log')->where(array('pid'=>$id,'status'=>1))->count();
}
//$id用户id $account用户投资金额 $v用户等级
function return_m($id){
       $map['pid']=$id;
       $map['type']=array("in",'3,10');
       $list=db('fund_log')->where($map)->select();
                         $m_3['addtime']=time();
                         $m_3['pid']=$id;
                         $m_3['act']='-';
       foreach ($list as $key => $value) {
           $r = db('users')->where('user_id', $value['uid'])->setDec('money', $value['money']);
           if ($r) {
                $m_3['uid']=$value['uid'];
                $m_3['type']=$value['type'];
                $m_3['money']=$value['money'];
                $m_3['note']='审核未通过，退还'.$value['note'];
               db('fund_log')->insert($m_3);
           }
       }
        return true;
}
//$v1本身等级 $v2上级等级 $pid上级id $money金额 $nickname用户名 $id投资id
function re_team($v1,$v2,$pid,$money,$nickname,$id){
                   $info=db('users')->field('pid')->find($pid);
                   if ($info['pid']) {
                        $pre=db('users')->field('level')->find($info['pid']);
                         $m_1['type']=10;
                         $m_1['uid']=$info['pid'];
                         $m_1['addtime']=time();
                         $m_1['pid']=$id;
                   ...
                   //投资人为普通会员的情况
                         ...
                         $m_3['act']='+';
                   if ($v1==$v2&&$v2>1) {
                                 $m_3['money']=round($money*0.01,2);
                                 $m_3['note']='获得团队会员投资1%的平级奖励，会员：'.$nickname;
                                 $r = db('users')->where('user_id', $pid)->setInc('money',$m_3['money']);
                                 if ($r) {
                                      db('fund_log')->insert($m_3);
                                 }
                   }else{     
                               if ($v2==2) {
                                     $m_['money']=round($money*0.03,2);
                                     $m_['note']='获得团队会员投资3%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $pid)->setInc('money',$m_['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_);
                                     }

                               }
                               if ($v2==3) {
                                     $m_['money']=round($money*0.05,2);
                                     $m_['note']='获得团队会员投资5%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $pid)->setInc('money',$m_['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_);
                                     }
                               ...
                               if ($v2==9&&$v1==2) {
                                     $m_['money']=round($money*0.12,2);
                                     $m_['note']='获得团队会员投资12%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $pid)->setInc('money',$m_['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_);
                                     }
                                ...
                                if ($v2==9&&$v1==3) {
                                     $m_['money']=round($money*0.1,2);
                                     $m_['note']='获得团队会员投资10%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $pid)->setInc('money',$m_['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_);
                                     }
                                ...
                                if ($v2==9&&$v1==1) {
                                     $m_['money']=round($money*0.15,2);
                                     $m_['note']='获得团队会员投资15%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $pid)->setInc('money',$m_['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_);
                                     }
                  ...
                  if (isset($pre['level'])) {
                   //上上级奖励
                       if ($info['pid']&&$pre['level']==$v2&&$v2>1) {
                                 $m_1['money']=round($money*0.01,2);
                                 $m_1['note']='获得团队会员投资1%的平级奖励，会员：'.$nickname;
                                 $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_1['money']);
                                 if ($r) {
                                      db('fund_log')->insert($m_1);
                                 }
                         }else{
                                if ($info['pid']&&$pre['level']==2) {
                                    $m_2['money']=round($money*0.03,2);
                                    $m_2['note']='获得团队会员投资3%的奖励，会员：'.$nickname;
                                    $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_2['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_2);
                                     }

                                 }
                                 if ($info['pid']&&$pre['level']==3) {
                                         $m_2['money']=round($money*0.05,2);
                                         $m_2['note']='获得团队会员投资5%的奖励，会员：'.$nickname;
                                         $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_2['money']);
                                         if ($r) {
                                              db('fund_log')->insert($m_2);
                                         }
                                 }
                                if ($info['pid']&&$pre['level']==9&&$v2==1) {
                                     $m_2['money']=round($money*0.15,2);
                                     $m_2['note']='获得团队会员投资15%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_2['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_2);
                                     }
                               }
                               if ($info['pid']&&$pre['level']==9&&$v2==2) {
                                     $m_2['money']=round($money*0.12,2);
                                     $m_2['note']='获得团队会员投资12%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_2['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_2);
                                     }
                               }
                                if ($info['pid']&&$pre['level']==9&&$v2==3) {
                                     $m_2['money']=round($money*0.1,2);
                                     $m_2['note']='获得团队会员投资10%的奖励，会员：'.$nickname;
                                     $r = db('users')->where('user_id', $info['pid'])->setInc('money',$m_2['money']);
                                     if ($r) {
                                          db('fund_log')->insert($m_2);
                                     }
